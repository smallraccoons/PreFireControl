...
<el-form :inline="true" class="demo-form-inline">
   ...
   <el-form-item>
    ...
    <el-form-item>
    请选择安全状态
    <el-select v-model="searchObj.riskLevel" placeholder="请选择安全状态">
      <el-option label="情况良好" value="0"></el-option>
      <el-option label="存在风险" value="1"></el-option>
    </el-select>
  </el-form-item>
...
:data="list" stripe style="width: 100%" @selection-change="handleSelectionChange">
    ...
    <el-table-column prop="riskLevel" label="是否危险"/>
...
<script>
//引入接口定义的js文件
import fireControlSubmit from '@/api/fireControlSubmit'
...
export default {
    data(){
        return {
            ...
            url: 'http://101.133.165.81:8080/draw/index.html',
    ...
    methods: {//定义方法，进行请求接口的调用
     toRescue(id) {
        ...
        lockFireControlSubmit(id,riskLevel){
            fireControlSubmit.lockFireControlSubmit(id,riskLevel)
             .then(response => {
                            //刷新页面
                            this.getList(1)
                        })
        ...
        getList2(page=1) {
            this.current = page
            this.searchObj.riskLevel = 1
            fireControlSubmit.getFireControlSubmitList(this.current,this.limit,this.searchObj)
        ...
        removeDataById(id) {
            this.$confirm('此操作将永久删除数据信息, 是否继续?', '提示', {
                ...
                type: 'warning'
                    ...
                    //调用接口
                    fireControlSubmit.deleteFireControlSubmit(id)
                        .then(response => {
                            //提示
                            this.$message({
                            type: 'success',
        ...
        removeRows() {
            this.$confirm('此操作将删除设备提交信息, 是否继续?', '提示', {
                ...
                //调用接口
                fireControlSubmit.batchdeleteFireControlSubmit(idList)
                    .then(response => {
                        //提示
                        this.$message({
                        type: 'success',
